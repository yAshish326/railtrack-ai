...
/**
 * Single WebClient gateway for all RailRadar APIs: the documented v1 endpoints
 * (train details, live status, route, journeys, station boards) plus the
 * legacy between-stations search endpoint used by the existing train-search
 * and AI-recommendation flows.
 */
@Component
public class RailRadarClient {
    ...
    public RailRadarClient(
            WebClient webClient, TrainApiProperties properties) {
        this.properties = properties;
        this.webClient = webClient.mutate().
                baseUrl(properties.getBaseUrl()).build();
    }

    /** Legacy train search between two stations (existing search + AI-recommendation API). */
    public TrainSearchResponse legacySearchTrains(String from, String to) {
        return webClient.get()
                .uri(builder -> builder.path("/legacy/trains/between")
    ...
    public RailRadarResponse liveTrain(
            String number, LocalDate date, boolean haltsOnly, boolean geometry, String format, boolean includeCoordinates) {
        return
                get("/trains/" + number + "/live", params("date", date, "haltsOnly", haltsOnly, "geometry", geometry, "format", format, "includeCoordinates", includeCoordinates));
    }
    ...
    public RailRadarResponse route(String number, String format, boolean stops) {
        return
                get("/trains/" + number + "/route", params("format", format, "stops", stops));
    }
    @Cacheable(value = "railRadarJourney", key = "#from + ':' + #to + ':' + #date + ':' + #live + ':' + #byCity + ':' + #type + ':' + #category")
    public RailRadarResponse betweenStations(String from, String to, LocalDate date, boolean live, boolean byCity, String type, String category) {
        return
                get("/trains/between/" + from + "/" + to, params("date", date, "live", live, "byCity", byCity, "type", type, "category", category));
    }
    ...
    public RailRadarResponse stationBoard(String code, boolean includeIntermediate) {
        return
                get("/stations/" + code + "/trains", params("includeIntermediate", includeIntermediate));
    }
    @Cacheable(value = "railRadarStationLiveBoard", key = "#code + ':' + #hours + ':' + #includeIntermediate")
    public RailRadarResponse stationLiveBoard(String code, int hours, boolean includeIntermediate) {
        return
                get("/stations/" + code + "/live", params("hours", hours, "includeIntermediate", includeIntermediate));
    }

    private RailRadarResponse get(String path, Map<String, Object> parameters) {
        try {
            return
                    webClient.get().uri(builder -> { var uri = builder.path(path); parameters.forEach((key, value) -> { if (value != null) uri.queryParam(key, value); }); return uri.build(); })
                    .headers(headers -> headers.setBearerAuth(propertiesKey()))
                    .retrieve()
                    .onStatus(status -> status.value() == 404, response -> response.createException().map(error -> new RailRadarClientException(HttpStatus.NOT_FOUND, "RailRadar resource was not found.", error)))
                    .onStatus(status -> status.isError(), response -> response.createException().map(error -> new RailRadarClientException(HttpStatus.BAD_GATEWAY, "RailRadar request failed with status " + error.getStatusCode().value() + ".", error)))
